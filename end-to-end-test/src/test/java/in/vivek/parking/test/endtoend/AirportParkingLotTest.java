...
import in.vivek.parking.feestrategies.AirportFeeStrategy;
...
public class AirportParkingLotTest extends ParkingLotTest {
    ...
    public void setup() throws IOException, URISyntaxException {
        ...
        ImmutableMap<SpotType, Integer> spotAvailability = ImmutableMap.of(
                SpotType.TWO_WHEEL, 200,
                SpotType.SMALL_FOUR_WHEEL, 500,
                SpotType.LARGE_FOUR_WHEEL, 100
        );
        // initialize parkingLot

        FeeModel<AirportFeeStructure> feeModel = objectMapper.readValue(TestUtil.readFileFromResources("airport_fee_model.json"), new TypeReference<FeeModel<AirportFeeStructure>>() {
        });
        FeeModelDao<AirportFeeStructure> feeModelDao = new InMemoryFeeModelDao<>(feeModel);
        FeeStrategy<AirportFeeStructure> feeStrategy = new AirportFeeStrategy();
    ...
    public void airPortParkingLotTest() {
        ...
        try {
            ...
            Instant outTime1 = inTime1.plus(Duration.ofMinutes(55));
            Receipt receipt1 = simpleParkingLot.unParkVehicle(ticket1.getTicketId(), outTime1);
            Assertions.assertEquals(0, receipt1.getFees());
            ...
            Instant outTime2 = inTime2.plus(Duration.ofMinutes(3 * 24 * 60 + 60));
            Receipt receipt2 = simpleParkingLot.unParkVehicle(ticket2.getTicketId(), outTime2);
            Assertions.assertEquals(400.0, receipt2.getFees());
            System.out.println(receipt1.toString());
